...
// static/htmx-echarts.js
var P = "componentType componentSubType seriesType seriesIndex seriesName name dataIndex value data color".split(" ");
var pick = (p) => {
...
var bridge = (chart, el) => {
  ...
  b = b == null || !String(b).trim() ? "click,hover" : String(b).trim().toLowerCase();
  if (b === "false" || b === "none")
    return;
  ...
  const fire = (n, d) => typeof htmx !== "undefined" && htmx.trigger ? htmx.trigger(el, n, d) : el.dispatchEvent(new CustomEvent(n, { bubbles: true, cancelable: true, detail: d }));
  if (onClick)
    chart.on("click", (p) => fire(cn, pick(p)));
  if (onHover)
    chart.on("mouseover", (p) => fire(hn, pick(p)));
};
var createChart = (el) => {
...
var parseDurationMs = (raw) => {
  ...
  if (!Number.isFinite(n) || n <= 0)
    return null;
  return (m[2] || "ms").toLowerCase() === "s" ? Math.round(n * 1000) : Math.round(n);
};
var parseDataUrl = (raw) => {
  const s = String(raw || "").trim();
  if (!s)
    return { url: "", pollMs: null };
  const parts = s.split(/\s+/).filter(Boolean);
  let pollMs = null;
  for (const tok of parts.slice(1)) {
    const m = tok.match(/^poll:(.+)$/i);
    if (m)
      pollMs = parseDurationMs(m[1]);
  }
  return { url: parts[0] || "", pollMs };
};
var alive = (chart) => chart && !chart.isDisposed?.();
var apply = (chart, option) => {
  if (!alive(chart))
    return;
  chart.setOption(option);
  chart.hideLoading();
};
var remoteFetch = (chart, url) => fetch(url).then((r) => {
  if (!r.ok)
    throw new Error(`${r.status} ${r.statusText}`.trim());
  return r.json();
}).then((d) => apply(chart, d)).catch((e) => {
  console.error("htmx-echarts fetch", url, e);
  if (alive(chart))
    chart.hideLoading();
});
var remoteSSE = (chart, url, ev) => {
...
(() => {
  ...
  const init = (el) => {
    ...
    if (!url)
      return console.error("htmx-echarts: missing data-url", el);
    const chart = createChart(el);
    bridge(chart, el);
    if (el.dataset.chartLoading !== "false")
      chart.showLoading();
    if (sse)
      el._sseSource = remoteSSE(chart, url, sse);
    else {
  ...
  const destroy = (el) => {
    el._sseSource?.close();
    if (el._pollIntervalId)
      clearInterval(el._pollIntervalId);
  ...
  htmx.registerExtension("echarts", {
    ...
    htmx_before_swap: (_elt, detail) => {
      for (const task of detail?.tasks ?? []) {
        ...
        if (style.includes("Morph") || style === "none")
          continue;
        cleanup(task.target);
      }
    },
